...
export class CoreDbMongoRepository extends MongoRepository implements CoreDbTrackingRepository {
  constructor(
  ...
  public async saveTracking(tracking: Tracking): Promise<boolean> {
    const db = (await this.pool.connect()).db(CoreConstants.MONGO_TRACKING_CORE_DB)
    const model: CoreTrackingModel = CoreTrackingModel.fromEntity(tracking)
    const result = await db.collection(CoreConstants.MONGO_TRACKING_CORE_COL).insertOne(model)
    if (!result.acknowledged) throw new CoreException('CoreMongoRepository Error')
    return true
  }

  public async getTracking(trackingNumber: string): Promise<Tracking> {
    const db = (await this.pool.connect()).db(CoreConstants.MONGO_TRACKING_CORE_DB)
    const result = await db.collection(CoreConstants.MONGO_TRACKING_CORE_COL).findOne({ trackingNumber })
    if (!result) throw new CoreException('Tracking does not exist')
    const tracking: Tracking = CoreTrackingModel.toEntity(result as CoreTrackingModel)
    return tracking
  }
}
