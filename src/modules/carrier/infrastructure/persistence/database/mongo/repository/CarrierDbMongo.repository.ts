...
export class CarrierDbMongoRepository extends MongoRepository implements CarrierDbRepository {
  constructor(
  ...
  public async saveDhlTrackingEvent(event: DhlTrackingEvent): Promise<boolean> {
    const db = (await this.pool.connect()).db(CarrierConstants.MONGO_TRACKING_CARRIER_DB)
    const cursor = await db.collection(CarrierConstants.MONGO_TRACKING_CARRIER_DHL_COL).insertOne(event)
    if (!cursor.acknowledged) throw new CoreException('CarrierMongoRepository error')
    return true
  }

  public async saveGlsTrackingEvent(event: GlsTrackingEvent): Promise<boolean> {
    const db = (await this.pool.connect()).db(CarrierConstants.MONGO_TRACKING_CARRIER_DB)
    const cursor = await db.collection(CarrierConstants.MONGO_TRACKING_CARRIER_GLS_COL).insertOne(event)
    if (!cursor.acknowledged) throw new CoreException('CarrierMongoRepository error')
    return true
  }
}
