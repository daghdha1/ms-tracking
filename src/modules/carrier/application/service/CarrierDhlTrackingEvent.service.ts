...
export class CarrierDhlTrackingEventService {
  ...
  public async run(dto: DhlTrackingEventDto): Promise<boolean> {
    const tracking: Tracking = await this.getTrackingService.run(
      dto.tracking_number,
    );
    const dhlTrackingEvent: TrackCreatedEvent = this.createEvent(dto, tracking);
    try {
      this.kafkaClient.emit(
        process.env.KAFKA_TRACKING_CARRIER_TOPIC,
